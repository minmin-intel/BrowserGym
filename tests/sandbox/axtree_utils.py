"""
Helper module for flattening AX tree.
"""

import ast

# Constants for AX tree processing
...
def _process_bid(
    ...
    """
    # Create an empty dict if no extra properties were provided
...
def flatten_axtree_to_str(
            ...
            if not skip_node:
                ...
                if not (
                    ...
                    or (
                        ...
                        and extra_properties and extra_properties.get(bid, {}).get("visibility", 0) < 0.5
                    )
                ):
    ...
    try:
        ...
        return f"Error flattening AX tree: {str(e)}"
